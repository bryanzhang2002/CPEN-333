...
def philosopher(id: int, chopstick: list, table_limit: multiprocessing.Semaphore): 
    """
       implements a thinking-eating philosopher by limiting the number of seats at the table
       id is used to identifier philosopher #id (id is between 0 to numberOfPhilosophers-1)
       chopstick is the list of semaphores associated with the chopsticks, table_limit semaphore is used to limit the table to four seats
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        table_limit.acquire()   # acquires a seat at the table
        ...
        table_limit.release() # Releases a seat at the table
...
if __name__ == "__main__":
    ...
    table_limit = multiprocessing.Semaphore(numberOfPhilosophers-1)  # semaphore that limits table to four philosophers
